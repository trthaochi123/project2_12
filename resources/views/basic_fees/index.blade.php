                    ...
                    <div class="table2">
                        <table id="myDataTable" cellspacing="0" cellpadding="0" width="100%">
                            ...
                            @foreach($basic_fees as $basic_fee)
                                <tr>
                                    <td>{{ $basic_fee->majorName }}</td>
                                    <td>{{ $basic_fee->academicName}}</td>
                                    <td>{{ number_format($basic_fee->basic_fee_amount)}}</td>
                                    <td class="btn-edit">
                                        <div class="edit-block">
                                            <a href="{{ route('basic_fees.edit',[$basic_fee->major_id,$basic_fee->academic_id]) }}">
                                                <i class="fas fa-pen"></i>
                                            </a>
                                        </div>
                                    </td>
                                    <td class="btn-delete">
                                        <form method="post" action="{{ route('basic_fees.destroy',[$basic_fee->major_id, $basic_fee->academic_id]) }}">
                                            @csrf
                                            @method('DELETE')
